...
sed "s|targetRevision: main|targetRevision: ${REV}|" \
  "${ROOT}/deploy/argocd/relay-application-dev.yaml" | kubectl apply -f -
echo "    Application 'relay' is tracking origin/${REV}"

echo "==> [4/4] wait for first sync"
for _ in $(seq 1 40); do
  sync="$(kubectl -n "${ARGOCD_NS}" get app relay -o jsonpath='{.status.sync.status}' 2>/dev/null || true)"
  health="$(kubectl -n "${ARGOCD_NS}" get app relay -o jsonpath='{.status.health.status}' 2>/dev/null || true)"
...
ArgoCD is installed and managing the 'relay' Application.
...
Demo the GitOps loop:
  ...
  3. Force an immediate refresh (or wait ~3m for the poll):
       kubectl -n ${ARGOCD_NS} annotate app relay argocd.argoproj.io/refresh=hard --overwrite
